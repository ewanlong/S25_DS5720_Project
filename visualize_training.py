...
if not os.path.exists(history_dir):
    print("History directory does not exist. Please ensure the model is trained and saved.")
    exit()

# Allow the user to specify a filename
model_name = input("Enter the model name to visualize (without the .his extension, press enter to use args.save_path): ").strip()
if not model_name:
...
try:
    ...
    # Print detailed structure of metrics for debugging
    ...
    if isinstance(metrics, dict):
        ...
        for key, value in metrics.items():
            if isinstance(value, list):
                ...
                if len(value) > 0:
                    print(f"    First element type: {type(value[0])}, Value: {value[0]}")
                else:
                    print("    List is empty")
            else:
    ...
    print(f"Unable to load model history: {model_name}, Error: {str(e)}")
...
# Check the data structure and extract training loss data
if 'TrainLoss' in metrics and isinstance(metrics['TrainLoss'], list):
    # Use list indices as epoch numbers
    ...
    # Ensure the lists are of consistent lengths
    ...
    # Plot all available metrics (excluding loss-related metrics)
    ...
    if metric_keys:
        print(f"Found the following additional metrics: {metric_keys}")
        
        for key in metric_keys:
            if len(metrics[key]) == 0:
                print(f"Warning: Metric {key} has an empty data list")
            else:
                print(f"Metric {key} has {len(metrics[key])} data points")
        
        # Group metrics by training and testing prefixes
        ...
        # Process paired metrics (Training and Testing)
        for key in metric_keys:
            if key in processed_metrics:
                continue
                
            metric_name = key[5:] if key.startswith('Train') else key[4:]  # Remove 'Train' or 'Test' prefix
            ...
            if not has_train_data and not has_test_data:
                print(f"No data for metric {metric_name}, skipping plot.")
                continue
            ...
            if has_train_data:
                ...
                print(f"  Plotted {train_key} data with {train_data_len} points")
            ...
            if has_test_data:
                ...
                print(f"  Plotted {test_key} data with {test_data_len} points")
        ...
        # Create a combined chart displaying all non-empty metrics
        ...
        if len(non_empty_metrics) > 1:
            ...
            print(f"Combined metrics curve saved to {output_dir + model_name + '_all_metrics.png'}")
else:
    print("Expected data structure not found. Please check file format.")
    print("Expected keys: 'TrainLoss', 'TrainpreLoss', etc.")
